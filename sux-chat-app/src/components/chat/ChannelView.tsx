...
import {
  ...
  Trash2, Play, Square, ChevronLeft, UserPlus,
} from "lucide-react";
...
interface Post {
  id: string; content?: string; created_at: string; file_url?: string;
  video_url?: string; download_only?: boolean; sender?: { id: string; username: string };
...
/** Медиа поста: картинка или видео (s3:// разрешает useMediaUrl). */
const PostMedia = ({ post }: { post: Post }) => {
  const url = useMediaUrl(post.video_url || post.file_url);
  if (!url) return null;
  if (post.video_url) {
    return <video src={url} controls playsInline className="mt-2 max-h-80 w-full object-contain bg-black" />;
  }
  if (post.file_url && !post.download_only) {
    return <img src={url} alt="" className="mt-2 max-h-80 w-full object-contain" loading="lazy" />;
  }
  return null;
};
      ...
      {isAdmin ? (
        <div className="border-t border-border pad-safe-bottom px-3 py-2 shrink-0">
          ...
          <div className="flex items-end gap-2">
            <button type="button" onClick={openSounds} className={cn("w-10 h-10 shrink-0 flex items-center justify-center border border-border", sound && "text-primary border-primary")} aria-label="Звук пуша">
