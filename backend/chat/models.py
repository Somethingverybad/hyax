...
class PushDevice(models.Model):
    """APNs-токен установки приложения. На профиль может быть несколько
    устройств; невалидные токены деактивируются при отправке."""
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    profile = models.ForeignKey(Profile, on_delete=models.CASCADE, related_name='push_devices')
    token = models.CharField(max_length=200, unique=True)
    platform = models.CharField(max_length=16, default='ios')
    is_active = models.BooleanField(default=True)
    updated_at = models.DateTimeField(auto_now=True)

    def __str__(self):
        return f"{self.profile.username} · {self.platform} · {self.token[:12]}…"
