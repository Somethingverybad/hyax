...
class UserConsumer(AsyncWebsocketConsumer):
    ...
    # Клиент шлёт {type:'rov', chat:<id>, on:true} пока держит палец и
    # {on:false} при отпускании. Каким будет отклик — одиночный тычок или
    # сплошной гул — решает приёмник по длительности удержания (см.
    # sux-chat-app/src/lib/rov.ts); поле rate осталось от прежней версии,
    # его просто передаём дальше, старые клиенты его ещё шлют.
    #
    ...
    def push_rov(self, targets, from_name, from_id):
        ...
        if not offline:
            logger.info("rov: пуш не нужен — все в приложении либо уже получили")
            return
        logger.info("rov: шлю пуш %d из %d (их нет в приложении)", len(offline), len(targets))
